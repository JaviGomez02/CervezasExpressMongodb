...
if (!db.cervezas.find().length) {
 ...
 if (!db.bares.find().length){
    const bares=[{
        ...
        "descripcion":"Un bar de mierda que se está poniendo muy caro"
    }, {
        "Nombre":"El piti",
    "descripcion":"Na bueno"
    }];
    db.bares.save(bares);
 }
 if (!db.usuarios.find().length){
    const usuarios=[{"Nombre":"Javi",
    "apellidos":"Gomez",
    "nick":"Javibu",
    "email":"javigomez055@gmail.com",
    "contrasena":"javi1234"
    }]
